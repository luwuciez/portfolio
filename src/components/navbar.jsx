...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    if (menuIsOpen && mediaQuery.matches) {
  ...
  return (
    <div className="font-dela flex items-center justify-between mx-6 md:mx-8 my-4">
      ...
      <nav className="flex flex-row bg-surface/80 backdrop-blur-xs p-1 rounded-full shadow-custom border border-light/10">
        <div
        ...
        <div ref={menuRef} className="overflow-hidden">
          <ul ref={ulRef} className="flex flex-row space-x-2 py-2">
            {tabs.map((tab) => (
              <li key={tab.name}>
                <Link
                  to={tab.path}
